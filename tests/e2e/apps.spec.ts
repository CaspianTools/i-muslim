...
test.describe("Quran app page", () => {
  ...
  test("the changelog page lists every shipped release", async ({ page }) => {
    ...
    // 42 shipped versionCodes; the two unshipped ones are excluded on purpose.
    await expect(rows).toHaveCount(42);
    await expect(page.getByText("1.0", { exact: true }).last()).toBeVisible();
  });

  test("/apps redirects to the single app page", async ({ page }) => {
    await page.goto("/en/apps");
    await expect(page).toHaveURL(/\/en\/apps\/quran$/);
  });
