...
### Added
- **Admin Quran reader: per-translation show/hide filter** — the `/admin/quran/<surah>` page now has a searchable multi-select language combobox next to the text filter, mirroring the control on `/admin/hadith/<collection>`. Toggling English or Russian hides that translation block on every ayah card; Arabic and the transliteration always render. Selection persists per browser via `localStorage` (`i-muslim.admin-quran-visible-langs`, distinct from the hadith key). The editor drawer is unaffected — both EN and RU remain editable regardless of the filter.
