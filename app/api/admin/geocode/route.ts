  ...
  if (db) {
    try {
      ...
      if (
        ...
        return NextResponse.json({ ok: true, lat: data.lat, lng: data.lng, timezone: data.timezone, cached: true });
      }
    } catch {
      // ignore cache errors
    }
  ...
  if (db) {
    try {
      await db
        ...
        .doc(cacheKey)
        .set({ lat, lng, timezone, query: q, cachedAt: Timestamp.now() }, { merge: true });
    } catch {
      // ignore cache write errors
    }
  }

  return NextResponse.json({ ok: true, lat, lng, timezone, displayName: results[0]!.display_name });
}
