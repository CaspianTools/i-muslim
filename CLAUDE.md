...
`/apps/quran` is the product page for the **i-muslim Quran** Android app, which lives in the sibling repo `i-muslim-Quran`. Two things there feed this repo, and neither is automated — there is no CI and no cross-repo sync:

- **[lib/apps/quran.ts](lib/apps/quran.ts)** holds the release history and the app constants. It is hand-maintained: a new Play release means prepending one entry. That single edit drives `/apps/quran`, `/apps/quran/changelog`, the JSON-LD, and the sitemap `lastModified`. Release notes are **English only** and must never move into `messages/*.json` — ~40 permanently-untranslated keys would destroy the `check:locales` signal. The page copy under the `apps` namespace comes from that repo's `play-assets/store-listing.html`, which is already written in en/ar/tr/id.
