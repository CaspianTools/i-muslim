...
There are two app product pages, plus a `/apps` index that lists both:

| Page | App | Sibling repo | Token | Revalidate |
|---|---|---|---|---|
| `/apps/quran` | **Quran by i-muslim** (`com.imuslim.quran`) | `i-muslim-Quran` | `QURAN_APP_REPO_TOKEN` | `POST /api/revalidate/quran-app` |
| `/apps/prayer` | **Salatuk (Namaz) Prayer Times** (`com.imuslim.prayer`) | `i-muslim-prayer` | `PRAYER_APP_REPO_TOKEN` | `POST /api/revalidate/prayer-app` |

The prayer app is branded **Salatuk** — never "i-muslim Prayer Times", which is its pre-1.7.0 name. Each page's `hero.title` is the exact Play store name *for that locale*, so it matches what a Play reviewer sees.

- **Release history is fetched live from GitHub.** [lib/apps/releases.ts](lib/apps/releases.ts) exposes `makeReleaseLoader`, which reads the `RELEASES` array out of an app repo's `play-assets/store-listing.html` (each app's single release source of truth, updated on every release by that repo's workflow), parses it with [lib/apps/store-listing.ts](lib/apps/store-listing.ts), and caches it for an hour — so a new Play release shows up on the app page, its changelog, the JSON-LD, and the sitemap `lastModified` on its own. [quran-releases.ts](lib/apps/quran-releases.ts) and [prayer-releases.ts](lib/apps/prayer-releases.ts) are thin wrappers over it. Both repos are private: each fetch needs its own token (see the table above and [apphosting.yaml](apphosting.yaml)), and its revalidate endpoint with `REVALIDATE_SECRET` refreshes it instantly. Without the token, or on any fetch/parse failure, the page falls back to the committed snapshot in [lib/apps/quran.ts](lib/apps/quran.ts) / [lib/apps/prayer.ts](lib/apps/prayer.ts) — refresh those occasionally so the fallback doesn't lag far behind. **Snapshots must contain only shipped releases**: the live fetch drops `shipped:0` entries, but a snapshot is merged in unfiltered, so a never-uploaded build pasted in there renders forever. Release notes are **English only** and must never move into `messages/*.json` — dozens of permanently-untranslated keys would destroy the `check:locales` signal. The page copy under each app's `apps.*` namespace comes from that repo's `play-assets/store-listing.html`, which is already written in en/ar/tr/id.
- **[legal.privacy.app](messages/en.json)** (Quran, `/privacy#android-app`) and **`legal.privacy.prayerApp`** (Salatuk, `/privacy#prayer-app`) are the apps' *hosted privacy policies* — the URL submitted to the Play Console. Google cross-checks it against the app's Data safety form, so if a release changes what the app does with data (a new permission, a sync or account feature, a new SDK), this page must change in the same commit, in all four locales, with a new "last updated" date. An inaccurate policy here is a compliance problem, not a copy bug.

Screenshots under `public/apps/<app>/shots/<versionCode>/` are generated by `npm run gen:app-shots -- --app <quran|prayer>` from the sibling checkout and **committed** — the build must never depend on those repos being present. A bare `npm run gen:app-shots` does every app whose checkout it can find. Write a new versioned directory per release rather than overwriting, and bump that app's `shotSetVersionCode`.

The full checklist lives in each app repo's `CLAUDE.md`, in its release workflow.
