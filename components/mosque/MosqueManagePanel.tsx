...
import { Copy, Download, FileText, ImagePlus, ImageUp, Loader2, QrCode, Rocket, Save, Settings2, X } from "lucide-react";
...
export function MosqueManagePanel({
  ...
  return (
    <section className="rounded-xl border-2 border-dashed border-accent/40 bg-accent/5 p-5">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
      ...
      >
        <span className="inline-flex items-center gap-2 text-base font-semibold text-foreground">
          <Settings2 className="size-4 text-accent" /> {t("title")}
          {isDraft && (
            <span className="rounded bg-warning/20 px-2 py-0.5 text-xs font-medium text-foreground">
              {t("draftBadge")}
            </span>
          )}
        </span>
        <span className="text-xs text-muted-foreground">{open ? t("hide") : t("manage")}</span>
      </button>
        ...
        <div className="mt-4 space-y-6">
          {isDraft && (
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-card p-4">
              ...
              <Button onClick={handlePublish} disabled={saving === "publish"}>
          ...
          {mosque.shortCode && (
            <div className="space-y-2 rounded-lg border border-border bg-card p-4">
              ...
              <div className="flex flex-wrap gap-2">
                <a
                  href={`/m/${mosque.shortCode}/poster`}
                ...
                <a
                  href={`/m/${mosque.shortCode}/qr?format=png`}
                  className="inline-flex h-8 items-center gap-1.5 rounded-md border border-input bg-background px-3 text-sm hover:bg-muted"
                >
                  <Download className="size-4" /> {t("downloadPng")}
                </a>
                <a
                  href={`/m/${mosque.shortCode}/qr?format=svg`}
                  className="inline-flex h-8 items-center gap-1.5 rounded-md border border-input bg-background px-3 text-sm hover:bg-muted"
                >
          ...
          <div className="space-y-2">
            ...
            />
            <SaveButton label={t("save")} busy={saving === "about"} onClick={() => run("about", () => updateMosqueAbout(mosque.slug, about))} />
          </div>
