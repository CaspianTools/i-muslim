...
export function HadithList({
  ...
  const tList = useTranslations("hadithAdmin.list");
  const tStatus = useTranslations("hadithAdmin.status");
          ...
          >
              ...
              <div className="flex items-center gap-2">
                ...
                {draftLangs.length > 0 && (
                  <span
                    ...
                    title={tList("draftTranslations", {
                      langs: draftLangs.map((l) => l.toUpperCase()).join(", "),
                    })}
                  >
                    {tList("needsReview", { count: draftLangs.length })}
                  </span>
                )}
              </div>
              <Button
                ...
                aria-label={tList("editAria", { number: h.number })}
              >
                <Pencil /> {tCommon("edit")}
              </Button>
            </div>
            {h.narrator && (
              <p className="mb-2 text-xs italic text-muted-foreground">
                {tList("narratedBy", { narrator: h.narrator })}
              </p>
            )}
            <p
              dir="rtl"
...
function EditHadithDrawer({
  ...
  const tEd = useTranslations("hadithAdmin.editor");
  ...
  async function onSubmit(values: Values) {
    ...
    try {
      ...
      onSaved(updated);
      toast.success(tEd("toastSaved", { collection, number: hadith.number }));
    } catch (err) {
      ...
      <EditorDialogContent>
        {hadith && (
          ...
          >
            <EditorDialogHeader>
              <EditorDialogTitle>
                {tEd("title", { collection, number: hadith.number })}
              </EditorDialogTitle>
              <EditorDialogDescription>
                {tEd("description")}
              </EditorDialogDescription>
...
function useActionErrorText() {
  const t = useTranslations("hadithAdmin.translationField");
  return (code: string) => {
    switch (code) {
      case "invalid-input":
...
function TranslationField({
  ...
  const tTf = useTranslations("hadithAdmin.translationField");
        ...
        <div className="flex items-center gap-2">
          {canPublish && (
            <PublishToggle
              ...
              ariaLabel={tTf("publishStatusAria", { language: label })}
            />
          )}
          {editable && (
            <Button
