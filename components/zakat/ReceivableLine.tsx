...
import type { ReceivableEntry } from "@/lib/zakat/types";
import { cn } from "@/lib/utils";

interface Props {
  entry: ReceivableEntry;
  onUpdate: (id: string, updates: Partial<ReceivableEntry>) => void;
  onRemove: (id: string) => void;
}

export function ReceivableLine({ entry, onUpdate, onRemove }: Props) {
  const t = useTranslations("zakat");
  const isUSD = entry.currency === "USD";

  return (
    <div className="grid grid-cols-[1.5fr_1fr_8rem_7rem_auto] gap-2 items-end animate-in fade-in slide-in-from-top-1 duration-200">
      <Field label={t("fields.personName")}>
        <Input
          type="text"
          value={entry.name}
